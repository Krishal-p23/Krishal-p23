# 👋 Hi, I'm Krishal Prasad

I am a Computer Science and Engineering undergraduate at **NIT Delhi**, passionate about building intelligent systems that can learn from data and solve real-world problems. My interests lie at the intersection of **Machine Learning, Deep Learning, Computer Vision, and Data-Driven Systems**.

I enjoy exploring how AI can bridge the gap between research and practical applications, especially in domains like **biomedical AI, geospatial intelligence, and brain-computer interfaces**.

---

## 🚀 About Me

- 🎓 B.Tech CSE student at NIT Delhi (Batch 2023–2027)  
- 🧠 Interested in Machine Learning, Deep Learning & AI Research  
- 👁️ Focused on Computer Vision and Scientific AI applications  
- 🌍 Enthusiastic about building systems that impact society meaningfully  
- 💻 Full-stack + AI developer exploring scalable intelligent systems  
- 📚 Constantly learning and experimenting with new technologies  

---

## 🛠️ Tech Stack

<p align="center">
    <img src="https://skillicons.dev/icons?i=c,cpp,python,js,dart"/>
</p>
<p align="center"> 
    <img src="https://skillicons.dev/icons?i=nextjs,react,tailwind,html,css,flutter" /> 
</p> 
<p align="center"> 
    <img src="https://img.shields.io/badge/PrismJS-Code_Highlighting-purple?style=for-the-badge"/> 
    <img src="https://img.shields.io/badge/React_Markdown-Markdown_Renderer-orange?style=for-the-badge"/> 
</p>

<p align="center"> 
    <img src="https://skillicons.dev/icons?i=nodejs,mongodb,django,flask,mysql,postgres" /> 
</p> 
<p align="center"> 
    <img src="https://img.shields.io/badge/Mongoose-ODM_Library-green?style=for-the-badge"/> 
    <img src="https://img.shields.io/badge/Django%20REST-Framework-red?style=for-the-badge"/> 
</p>
<p align="center">
    <img src="https://img.shields.io/badge/OpenAI-000000?style=for-the-badge"/> 
    <img src="https://img.shields.io/badge/Groq-AI_Inference-FF6B00?style=for-the-badge"/> 
</p>

<p align="center"> 
    <img src="https://skillicons.dev/icons?i=tensorflow,pytorch,sklearn" /> 
</p> 
<p align="center"> 
    <img src="https://img.shields.io/badge/YOLO-Object%20Detection-blue?style=for-the-badge" /> 
    <img src="https://img.shields.io/badge/ViT-Vision%20Transformer-purple?style=for-the-badge" /> 
</p>
<p align="center">  
    <img src="https://img.shields.io/badge/Albumentations-Image%20Augmentation-orange?style=for-the-badge" /> 
    <img src="https://img.shields.io/badge/LabelMe-Annotation%20Tool-green?style=for-the-badge" /> 
</p>

<p align="center"> 
    <img src="https://skillicons.dev/icons?i=vercel,git,github,vscode,visualstudio" />
</p>
<p align="center">
    <img src="https://skillicons.dev/icons?i=androidstudio,figma,linux,vim" /> 
</p> 
<p align="center"> 
    <img src="https://img.shields.io/badge/Clerk-Auth_&_User_Management-3B82F6?style=for-the-badge"/> 
</p>

<p align="center"> 
    <img src="https://img.shields.io/badge/Axios-blue?style=for-the-badge"/> 
    <img src="https://img.shields.io/badge/React_Hot_Toast-Toast_Notifications-red?style=for-the-badge"/> 
</p>
---

## 📌 Projects

### 🛰️ SOHO Magnetogram Enhancement (Deep Learning)
Improved resolution of solar magnetogram images using **Deep Convolutional Neural Networks**, working with scientific imaging data and exploring model reliability in real-world research scenarios.

📄 Paper Link: https://link.springer.com/article/10.1007/s11207-026-02619-z

### 🧭 TourSort
A smart travel planning and sorting platform designed to help users efficiently organize and optimize their travel itineraries. The system focuses on improving travel planning through structured recommendations and user-centric design.

🔗 Repository: https://github.com/krishal-p23/toursort

### 🛠️ ServiGO – Service Provider App
A full-stack service marketplace application that connects users with local service providers. The platform streamlines service discovery, booking, and management, aiming to improve accessibility and convenience for everyday services.

🔗 Repository: https://github.com/krishal-p23/serviGO

---

# 📄 My Resumes

I maintain two tailored resumes based on roles:

## 💻 Software Development Engineer (SDE)
👉 [Download SDE Resume](https://krishal-p23.tiiny.site/KrishalResumeSDE.pdf)

Focused on:
- Full Stack Development
- System Design
- Backend Engineering
- Scalable Web Apps

---

## 🤖 AI/ML & Computer Vision Research
👉 [Download AI/ML Resume](https://krishal-p23.tiiny.site/KrishalResume.pdf)

Focused on:
- Machine Learning & Deep Learning
- Computer Vision (YOLO, ViT)
- AI Integrations & Research Projects
- Model Training & Deployment

---

## 🤝 Let’s Connect

I am always open to collaboration, research discussions, and learning opportunities in Machine Learning, AI, and Full Stack Development.

<p align="center">
  <a href="mailto:prasad.krishal23@gmail.com">
    <img src="https://skillicons.dev/icons?i=gmail" />
  </a>
  &nbsp;&nbsp;
  <a href="https://www.linkedin.com/in/krishal-p23">
    <img src="https://skillicons.dev/icons?i=linkedin" />
  </a>
</p> 
<!---
Krishal-p23/Krishal-p23 is a ✨ special ✨ repository because its `README.md` (this file) appears on your GitHub profile.
You can click the Preview link to take a look at your changes.
--->
